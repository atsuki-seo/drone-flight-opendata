...
```
{input_dir}/
├── {flight_id}{csv_suffix}.csv      # フラット構造（直下配置）
├── {flight_id}.SRT
├── {group}/                          # 任意名のサブディレクトリ（省略可）
│   ├── {flight_id}{csv_suffix}.csv
│   └── {flight_id}.SRT
└── {flight_id}-HD.mp4               # 動画（オプション）
```

- `{flight_id}`: フライト識別子。`^[a-z][a-z0-9-]+$` に準拠する任意の文字列（例: `sample-route1-1`, `tokyo-survey-001`）
- `{csv_suffix}`: CSVファイル名のサフィックス（CLI `--csv-suffix` で指定、デフォルト: `-テレメトリ`）
- `{group}`: 任意名のサブディレクトリ（整理用、コンバータは名前を解釈しない）
- CSV/SRTは同じ `{flight_id}` プレフィックスでペアリングされる
- SRT拡張子は `.SRT` `.srt` のどちらでも可
...
`{flight_id}{csv_suffix}.csv`（例: `sample-route1-1-テレメトリ.csv`, `flight-001.csv`）
...
2025-01-01T00:00:00.000000+00:00,35.620000,139.780000,328.08399,,,,0.0,0.0,0.0,0.0,
2025-01-01T00:00:01.002000+00:00,35.620000,139.780000,328.08399,,,,0.0,0.0,0.0,0.0,
```
...
`{flight_id}.SRT` または `{flight_id}.srt`（例: `sample-route1-1.SRT`）
...
`{flight_id}-HD.mp4`（例: `sample-route1-2-HD.mp4`）
